...
- Map layers: `faintA/B` (full tracks, opacity 0.28 — hidden unless "Show full track" toggle), `trailA/B` (bright ±10 s lead/lag around cursor: `sec*.slice(idx-10, idx+11)` clipped to the trim window), `dotA/B` (`circleMarker` + permanent SOG badge above each, `.sog-badge` tooltip via `setTooltipContent`), `windwardMarker` (draggable divIcon ★), `lineA/B` (dashed to mark), `mkAS/mkAE/mkBS/mkBE` (start/end markers, repositioned to trim window bounds when trimmed).
- Controls (row 2, `#trim-row`): **Cut start** / **Cut end** set the trim window at the current frame; **Reset trim** clears it; **Show full track** toggles the faint full-track override. Trim window `[trimStart, trimEnd)` clamps slider min/max, trails, dots, markers, and elapsed (`T+` is window-relative).
